...
def batch_trainer(epoch, model, train_loader, criterion, optimizer):
    ...
    # for step, (imgs, i, gt_label, id) in enumerate(train_loader):
    for step, (imgs, gt_label, imgname) in enumerate(train_loader):
        ...
        log_interval = 20
        step_loss.append(loss_meter.val)
    ...
    print('write epoch: %s train information ' % (epoch))
    with open('./exp_result/PA100k/train_step_info.txt', 'a+') as wf:
        for info in step_loss:
...
def valid_trainer(model, valid_loader, criterion):
    ...
    gt_list = []
    with torch.no_grad():
        # for step, (imgs, i, gt_label, id) in enumerate(valid_loader):
