...
class Train_Dataset(data.Dataset):
    ...
    def __init__(self, data_dir, dataset_name, transforms=None, train_val='train'):
        ...
        train = {}
        test_attr = {}
        trainset, query, gallery = import_MarketDuke_nodistractors(data_dir, dataset_name)
        train['data'] = trainset['data'] + gallery['data']
        train['ids'] = trainset['ids'] + gallery['ids']
        ...
        if dataset_name == 'Market-1501':
            ...
            train_attr, gallery_attr, self.label = import_DukeMTMCAttribute_binary(data_dir)
            # print("111111",self.label)
            train_attr = dict(train_attr, **gallery_attr)
        else:
            print('Input should only be Market1501 or DukeMTMC')

        for id in query['ids']:
            test_attr[id] = gallery_attr[id]
        ...
        for train_id in train['data']:
            label = train_attr[train_id[2]]
            distribution += np.array(label)

        self.distribution = distribution / len(train['data'])
        self.label_smooth_p = np.ones(self.num_labels) - 0.1
        self.label_smooth_n = np.ones(self.num_labels) * (0.1/self.num_labels)
        if train_val == 'train':
    ...
    def __getitem__(self, index):
        ...
        mask = label == 1
        label = label * mask * self.label_smooth_p
        label = label + np.bitwise_not(mask) *self.label_smooth_n
        data = Image.open(img_path)
        data = self.transforms(data)
        # name = self.train_data[index][4]
        return data, i, label, id
