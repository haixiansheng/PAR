...
from config_cxp import argument_parser
from dataset.AttrDataset_cxp import AttrDataset, get_transform
...
def main(args):
    ...
    save_model_path = os.path.join(model_dir, 'cxp_person_attr_resnet18_4cls.pth')
...
def trainer(epoch, model, train_loader, valid_loader, criterion, optimizer, lr_scheduler,
    ...
    for i in range(epoch):
        ...
        with open('./exp_result/PA100k/train_epoch_info_cxp.txt', 'a+') as wf:
                wf.writelines(str(train_loss) + '\n')
        with open('./exp_result/PA100k/valid_epoch_info_cxp.txt', 'a+') as wf:
    ...
    torch.save(result_list, os.path.join(os.path.dirname(path), 'metric_log_cxp.pkl'))
